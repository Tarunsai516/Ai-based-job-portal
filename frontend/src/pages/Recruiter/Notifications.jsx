...
export default function RecruiterNotifications() {
  ...
  useEffect(() => {
    notificationService.getByRole('recruiter')
      .then(data => {
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Recruiter Notifications</h1>
            <p className="text-xs text-gray-500 mt-0.5">Track candidate submissions and application status changes.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-8 text-center space-y-2 shadow-sm">
            ...
            <p className="text-xs text-gray-500">You'll be notified when candidates apply to your posted jobs.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {notifications.map(n => (
